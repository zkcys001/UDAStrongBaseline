...
class CrossBatchMemory(torch.nn.Module):
    def __init__(self, loss=ContrastiveLoss(), embedding_size=2048, memory_size=8192, miner=None):
        super().__init__()
        self.loss = loss
        self.miner = miner
    ...
    def create_indices_tuple(self, batch_size, embeddings, labels, E_mem, L_mem, input_indices_tuple):
        ...
        if input_indices_tuple is not None:
            ...
            indices_tuple = tuple(
                [torch.cat([x, y.to(x.device)], dim=0) for x, y in zip(indices_tuple, input_indices_tuple)])
    ...
    def shift_indices_tuple(self, indices_tuple, batch_size):
        if len(indices_tuple) == 3:
            ...
            indices_tuple = tuple(
                [x + batch_size if len(x) > 0 and i % 2 == 1 else x for i, x in enumerate(indices_tuple)])
        return indices_tuple